...
const LogInvoiceSummaryView = ({ onBackClick }) => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (years.includes(currentYear)) {
            ...
            setYear(years[0]); // Set first available year
          }
        } else {
  ...
  const handleExcelDownload = () => {
    ...
    if (data.length > 0) {
      const invoiceSummarySheet = XLSX.utils.json_to_sheet(
        data.map(item => ({
          'Loading Month': item.LOADING_MONTH || '',
          'Month Name': item.MONTHS || '',
          'No of Logs': item.NO_OF_LOGS || 0,
          'Invoice CBM': item.INVOICE_CBM || 0,
          'Total Value': item.TOTAL_VALUE || 0,
          'Average Rate': item.AVERAGE_RATE || 0
        }))
      );
      XLSX.utils.book_append_sheet(wb, invoiceSummarySheet, 'Log Invoice Summary');
    }

    XLSX.writeFile(wb, `Log_Invoice_Summary_${year}.xlsx`);
  };
  ...
  const fetchData = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/order/execute`, {
        ...
        body: JSON.stringify({
          procedure: 'proc_getloginvcoicesumyear',
          parameters: {
            year: parseInt(year)
          }
        })
      ...
      let invoiceSummaryData = apiData.data?.[0] || [];
      ...
      invoiceSummaryData = sortDataByMonth(invoiceSummaryData);

      setData(invoiceSummaryData);
    } catch (e) {
  ...
  const calculateSummaryStats = (data) => {
    ...
    const totalInvoiceCBM = data.reduce((sum, item) => sum + (parseFloat(item.INVOICE_CBM) || 0), 0);
    ...
    const avgCBMPerMonth = data.length > 0 ? totalInvoiceCBM / data.length : 0;
    const avgValuePerMonth = data.length > 0 ? totalValue / data.length : 0;
    const overallAverageRate = totalInvoiceCBM > 0 ? totalValue / totalInvoiceCBM : 0;

    // Find highest and lowest rate months
    const ratesData = data.map(item => ({
      month: item.MONTHS,
      rate: parseFloat(item.AVERAGE_RATE) || 0
    })).filter(item => item.rate > 0);

    const highRateMonths = ratesData.filter(item => item.rate > overallAverageRate).length;
    const lowRateMonths = ratesData.filter(item => item.rate <= overallAverageRate).length;
  ...
  const prepareChartData = (data) => {
    ...
    const invoiceCBM = data.map(item => parseFloat(item.INVOICE_CBM) || 0);
    const totalValues = data.map(item => parseFloat(item.TOTAL_VALUE) || 0);
    const averageRates = data.map(item => parseFloat(item.AVERAGE_RATE) || 0);
        ...
        <CardContent sx={{ p: 4 }}>
          <Box display="flex" alignItems="center" mb={3}>
            ...
            <Box sx={{ 
              ...
              <ReceiptIcon sx={{ fontSize: 28, color: '#ffffff' }} />
            </Box>
            <Box>
              ...
              <Typography variant="body2" sx={{ color: '#666', mt: 0.5 }}>
                Monthly log invoice analysis and financial trends
              </Typography>
            </Box>
          </Box>
          ...
          <CardContent sx={{ py: 2 }}>
            <Box display="flex" justifyContent="space-between" alignItems="center" flexWrap="wrap" gap={2}>
              <Typography variant="h6" sx={{ 
                ...
                <ReceiptIcon />
                Log Invoice Summary for {year} ({data.length} months)
              </Typography>
              <Button
                variant="contained"
            ...
            <Grid item xs={6} sm={6} md={3}>
              <Card sx={{ 
                ...
                <CardContent sx={{ 
                  ...
                  <ReceiptIcon sx={{ fontSize: { xs: 28, md: 40 }, mb: 1, opacity: 0.8 }} />
              ...
              <Card sx={{ 
                ...
                <CardContent sx={{ 
                  ...
                  <Typography variant="h4" sx={{ 
                    ...
                    {formatNumber(summaryStats.totalInvoiceCBM)}
                  </Typography>
                  <Typography variant="body2" sx={{ 
                    opacity: 0.9,
                    fontSize: { xs: '0.7rem', md: '0.875rem' }
            ...
            <Grid item xs={6} sm={6} md={3}>
              <Card sx={{ 
                ...
                <CardContent sx={{ 
                  ...
                  <AttachMoneyIcon sx={{ fontSize: { xs: 28, md: 40 }, mb: 1, opacity: 0.8 }} />
                  <Typography variant="h4" sx={{ 
                    fontWeight: 700, 
                    mb: 1,
                    fontSize: { xs: '1.2rem', md: '2rem' },
                    ...
                    {formatNumber(summaryStats.totalValue / 100000)}L
                  </Typography>
                  <Typography variant="body2" sx={{ 
                    opacity: 0.9,
                    fontSize: { xs: '0.7rem', md: '0.875rem' }
              ...
              <Card sx={{ 
                ...
                <CardContent sx={{ 
                  ...
                  <Typography variant="h4" sx={{ 
                    ...
                    {formatNumber(summaryStats.overallAverageRate)}
                  </Typography>
                  <Typography variant="body2" sx={{ 
                    opacity: 0.9,
                    fontSize: { xs: '0.7rem', md: '0.875rem' }
            ...
            <CardContent sx={{ height: '100%', p: { xs: 2, md: 3 } }}>
              ...
              <Box sx={{ 
                ...
                <Box sx={{ 
                  ...
                  <Bar
                    key={`value-cbm-chart-${year}`}
                    data={{
                      labels: chartData.months,
                      datasets: [
                        {
                          label: 'Invoice CBM',
                          data: chartData.invoiceCBM,
                          backgroundColor: 'rgba(27, 67, 50, 0.8)',
                          borderColor: 'rgba(27, 67, 50, 1)',
                          borderWidth: 1,
                          barThickness: barThickness,
                        ...
                        },
                        {
                          ...
                          data: chartData.totalValues.map(val => val / 100000), // Convert to lakhs
            ...
            <CardContent sx={{ height: '100%', p: { xs: 2, md: 3 } }}>
              ...
              <Box sx={{ 
                ...
                <Box sx={{ 
                  ...
                  <Line
                    key={`rate-trend-chart-${year}`}
                    data={{
                      labels: chartData.months,
                      datasets: [
                        {
                          label: 'Average Rate per CBM',
                          data: chartData.averageRates,
                          borderColor: 'rgba(255, 183, 77, 1)',
                          backgroundColor: 'rgba(255, 183, 77, 0.1)',
                          borderWidth: borderWidth,
                          fill: true,
                          tension: 0.4,
                          pointBackgroundColor: chartData.averageRates.map(val => 
                            val >= (summaryStats?.overallAverageRate || 0) ? 'rgba(16, 185, 129, 1)' : 'rgba(239, 68, 68, 1)'
                          ),
                          pointBorderColor: chartData.averageRates.map(val => 
                            val >= (summaryStats?.overallAverageRate || 0) ? 'rgba(16, 185, 129, 1)' : 'rgba(239, 68, 68, 1)'
                          ),
          ...
          <Card sx={{ 
            ...
            <CardContent sx={{ height: '100%', p: { xs: 2, md: 3 } }}>
              ...
              <Box sx={{ 
                ...
                <Doughnut
                  key={`rate-distribution-${year}`}
                  data={{
                    labels: ['Above Average Rate', 'Below Average Rate'],
                    datasets: [
                      {
                        data: [summaryStats.highRateMonths, summaryStats.lowRateMonths],
                        backgroundColor: [
                          'rgba(16, 185, 129, 0.8)',
                          'rgba(239, 68, 68, 0.8)',
                        ],
        ...
        <Card sx={{ 
          ...
          <TableContainer sx={{ maxHeight: '70vh' }}>
            <Table stickyHeader>
              ...
              <TableBody>
                {data.map((row, index) => {
                  const rate = parseFloat(row.AVERAGE_RATE) || 0;
                  const isAboveAverage = rate >= (summaryStats?.overallAverageRate || 0);
                  return (
                    <TableRow 
                      key={index}
                      sx={{ 
      ...
      {data && data.length === 0 && (
        <Card sx={{ borderRadius: 2, textAlign: 'center', py: 8 }}>
          <CardContent>
            ...
            <Typography variant="h6" color="textSecondary">
              No log invoice data found for {year}
            </Typography>
            <Typography variant="body2" color="textSecondary" sx={{ mt: 1 }}>
...
export default LogInvoiceSummaryView;