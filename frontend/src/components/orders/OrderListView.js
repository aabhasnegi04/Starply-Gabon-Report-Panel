...
const OrderListView = ({ onBackClick }) => {
  ...
  const handleExcelDownload = () => {
    ...
    XLSX.writeFile(workbook, `Order_List_${fromDate.format('YYYY-MM-DD')}_to_${toDate.format('YYYY-MM-DD')}.xlsx`);
  };
  ...
  const handleSearch = async () => {
    ...
    setData(null);
    try {
      const res = await fetch(`${API_URL}/order/listbydate`, {
        method: 'POST',
        ...
        <CardContent sx={{ p: 4 }}>
          <Box display="flex" alignItems="center" mb={3}>
            ...
            <Box sx={{ 
              ...
              <CalendarTodayIcon sx={{ fontSize: 28, color: '#ffffff' }} />
            </Box>
            <Box>
              <Typography variant="h4" sx={{ fontWeight: 700, color: '#1b4332', letterSpacing: '-0.5px' }}>
                Order List - Date Wise
              </Typography>
              <Typography variant="body2" sx={{ color: '#666', mt: 0.5 }}>
                Search and export orders by date range
              </Typography>
            </Box>
          </Box>
      ...
      {!data && !loading && !error && (
        <Card sx={{ 
          ...
          <Box sx={{
            ...
            <CalendarTodayIcon sx={{ fontSize: 50, color: '#1b4332' }} />
          </Box>
          <Typography variant="h5" sx={{ color: '#1b4332', fontWeight: 700, mb: 1 }}>
            No Orders to Display
          </Typography>
          <Typography variant="body1" sx={{ color: '#666', mt: 1 }}>
            Select a date range above and click Search to view orders
          </Typography>
        </Card>
      )}
    </Container>
